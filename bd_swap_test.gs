...
void main(){
	...
	float pId = curr_xyz.z*(width_ratio*height_ratio)+curr_xyz.y*width_ratio+curr_xyz.x;

	//8邻居
	float off_set[18] = 
	{-1.0/width_ratio,-1.0/height_ratio,
	0,-1.0/height_ratio,
	1.0/width_ratio,-1.0/height_ratio,
	-1.0/width_ratio,0,
	0,0,
	...
	float off_z[3]={-1.0/depth_ratio,0,1/depth_ratio};
	...
	cc0 = texture(clusterCov,vec3(0.5,curr_label+0.5,0.5)*c_whd_ratio).rgb;//位置处，横坐标加不加0.5都没问题，纵坐标不加0.5取得是上一行
	...
	neighbor_tex = this_tex + vec3(off_set[0],off_set[1],off_z[1]);
	...
	f_color = vec4(pId,curr_label,energy_from,energy_to);
	}else
	{
	energy_from=0;
